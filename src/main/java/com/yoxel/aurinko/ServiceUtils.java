...
//      scopes.replaceAll(s -> "Mail.Read".equalsIgnoreCase(s) ? "Mail.All" : s);
      ...
      try {
        final GenericJson
            sdData =
            Utils.getDefaultJsonFactory().createJsonParser(templ.getPassword()).parse(GenericJson.class);
    ...
    if (acc.isTrustServer() && acc.getTemplId() > 0) { // || forceManagedBy != null
      ...
      try {
        AurAccount svcAcc = AurinkoService
            ...
            .accounts
            .getMe();
        log.info("Upserting managed account " + acc.getId() + ", " + acc.getName() + ", " + acc.getEmailAddress()
      ...
      log.info(
          "Upserting account " + acc.getId() + ", " + acc.getName() + ", " + acc.getEmailAddress() + ", clientOrgId: "
  ...
  public static AurAccountToken syncTemplate(AurinkoService aurinko, String clientOrgId, ServiceTemplate svcTempl,
    ...
    log.info(
        "Upserting service account " + svcTempl.getId() + ", " + svcTempl.getName() + ", clientOrgId: " + clientOrgId
        + "/" + svcTempl.getGroupId());
